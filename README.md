...
## O. PWA — Cài app lên màn hình chính

App là PWA **tối thiểu**: cài được lên màn hình chính như app thật, chạy toàn
màn hình, **KHÔNG cache dữ liệu offline** (dữ liệu Google Sheets luôn realtime)
và **tự cập nhật** phiên bản mới sau khi deploy (không kẹt bản cũ, không cần
gỡ cài lại).

### Cài trên Android (Chrome)

1. Mở app bằng Chrome, đăng nhập.
2. Bấm nút **"⬇️ Cài ứng dụng"** hiện ở cuối màn hình (hoặc menu ⋮ →
   **Cài đặt ứng dụng** / **Thêm vào màn hình chính**).
3. Icon xuất hiện ở màn hình chính; mở từ icon → chạy toàn màn hình (không
   thanh địa chỉ).

### Cài trên iPhone/iPad (Safari)

Safari không có nút cài tự động — app sẽ hiện gợi ý ngắn ở cuối màn hình:

1. Mở app bằng **Safari** (bắt buộc — Chrome trên iOS không cài được).
2. Bấm nút **Chia sẻ** (ô vuông có mũi tên ↑) ở thanh dưới.
3. Chọn **"Thêm vào MH chính"** (Add to Home Screen) → **Thêm**.

### Thay logo nguồn cho icon

Icon hiện tại là **placeholder** chữ "LSX" trên nền xanh. Để dùng logo công ty:

1. Đặt file logo vào `public/logo-source.png` — ảnh **vuông**, độ phân giải
   cao (≥ 1024×1024). Icon maskable sẽ tự thêm ~20% vùng đệm an toàn quanh
   logo để không bị cắt khi hệ điều hành bo tròn.
2. Cài sharp (dev-only, chỉ dùng lúc sinh icon): `npm i -D sharp`
3. Chạy `npm run icons`.
4. Commit các file sinh ra (`public/icons/*`, `app/icon.png`,
   `app/apple-icon.png`) và deploy. Người dùng **không cần gỡ cài lại** —
   icon/manifest mới được hệ điều hành cập nhật dần.

### Kiến trúc PWA (để bảo trì)

| Thành phần | File | Ghi chú |
|---|---|---|
| Manifest | `app/manifest.ts` | Next phục vụ tại `/manifest.webmanifest` |
| Service worker | `public/sw.js` | KHÔNG cache HTML/API/dữ liệu — chỉ cache-first `/_next/static/*` và `/icons/*` (tên file có hash, không bao giờ cũ) |
| Đăng ký SW + tự cập nhật + nút cài | `components/pwa/pwa-setup.tsx` | SW mới nắm quyền → tự reload trang một lần; kiểm tra cập nhật mỗi lần app được focus |
| Sinh icon | `scripts/generate-icons.mjs` | Chạy qua `npm run icons` |
| Middleware | `middleware.ts` | Đã loại trừ `/sw.js`, `/manifest.webmanifest`, `/icons/*`… khỏi chặn đăng nhập |

**Nguyên tắc quan trọng (tránh kẹt bản cũ):** KHÔNG thêm precache app-shell
(kiểu next-pwa/Workbox mặc định) — cache-first cho HTML chính là nguyên nhân
kẹt bản cũ. Khi sửa `public/sw.js`, tăng version trong `CACHE_NAME`
(`lsx-static-v1` → `v2`) để bản mới dọn cache cũ.

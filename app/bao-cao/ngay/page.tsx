  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <ExportButtons fileName={`bao-cao-ngay-${ngay}.xlsx`} sheets={excelSheets} />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <section className="rounded-lg border border-gray-200 bg-white p-3">
          <h2 className="mb-2 text-sm font-semibold text-green-700">
            ✓ Đã xong trong ngày ({bc.daXongLenh.length} lệnh · {bc.soDonXong} đơn)
          </h2>
          <div className="space-y-2">
            {bc.daXongLenh.map((l) => (
              <div key={l.MaLenh} className="rounded-md border border-gray-100 p-2 text-sm">
            ...
            {bc.daXongLenh.length === 0 && (
              <p className="py-4 text-center text-xs text-gray-400">(chưa có)</p>
            )}
          </div>
        </section>
        ...
        <section className="rounded-lg border border-red-200 bg-red-50/40 p-3">
          ...
          <div className="space-y-2">
            {bc.nguyCoTre.map((n) => (
              <div key={n.MaLenh} className="rounded-md border border-red-100 bg-white p-2 text-sm">
                <div className="font-mono text-xs">{n.MaLenh}</div>
                <div className="font-medium">{n.TenSanPham || "—"}</div>
                <div className="text-xs text-gray-500">
                  {n.KhachHang} · Hạn {n.HanHoanThanh || "—"}
                  {n.ketThucDuKien && ` · dự kiến ${n.ketThucDuKien.slice(0, 10)}`}
                </div>
